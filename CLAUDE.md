...
## Ultracode Agentic Team

Model/effort per node: **global D272 standard** — `~/.claude/CLAUDE.md` ("Graph engineering", "Model-per-node allocation (D272)"); do not duplicate the table here. Local tightening: every substantive deliverable passes the Opus review gate (PASS/FAIL) — never merge past a failed gate.
...
- **Rate limiting**: `/api/chat` (code shipped, DORMANT — see below) enforces three independent limits via Upstash Redis: per-IP (10 req/60s), per-session (20 messages/conversation), per-day (1 new conversation/IP/day) — do not collapse these into a single counter (AR-17, REQ-F-066); unlike the forms below, all three FAIL CLOSED on a Redis error (`src/lib/rate-limit.ts` "Chatbot rate limiting" section — cost control is the point of the limits). Every other app-level limiter is IP-keyed and fail-**open** (a Redis outage must not block a legitimate form submission — REQ-I-020): `contact` 5/600s, `newsletter` 3/3600s, `newsletter_confirm` 10/3600s (its own, looser limiter — sharing `newsletter`'s 3/h would silently drop a second colleague's legitimate consent click from behind the same office NAT), `booking_webhook` 60/60s (checked AFTER HMAC verification), `company_lookup` 30/60s (search-as-you-type on the contact form's company autocomplete, not a one-shot submit). `company_lookup` additionally sits behind a GLOBAL (not per-IP) circuit breaker (`src/lib/circuit-breaker.ts`, Vlna 7) — 5 failures/60s per upstream (ARES/RPO tracked independently) trips a 2-minute skip-the-call cooldown, protecting Function compute-seconds from a slow/degraded upstream regardless of how many distinct IPs are hitting it. All POST routes with a body (`contact`, `newsletter`, `booking-webhook`, `chat`) also run a `Content-Length`-only body-size guard (`src/lib/body-size-guard.ts`) that 413s before Zod/HMAC ever sees an oversized payload. `/api/contact` and `/api/newsletter` have Vercel BotID (Basic tier, free, additive to Turnstile) coded and ready but shipped **DORMANT** behind `NEXT_PUBLIC_BOTID_ENABLED` (unset by default — do not flip without following `docs/security/abuse-surface.md` §5.4's activation checklist first; fails OPEN on error either way, never blocks on its own). A platform-layer Vercel WAF (custom rate-limit rules on `/api/*`, `/api/chat`, `/api/company-lookup`) is staged but **not yet published/enforcing** — see `docs/security/abuse-surface.md` for the full exposure map, the promotion steps, and the "under attack" runbook.
