...
- **AI calls**: Never import `@anthropic-ai/sdk` directly — all chatbot inference (deferred to Phase 5 per D-002; no `/api/chat` route exists yet) MUST go through the Vercel AI SDK + Vercel AI Gateway using `process.env.AI_MODEL` (AR-01, security-model.md Rule 2); direct Anthropic SDK calls bypass spend controls and model abstraction.
...
- **Rate limiting**: Once the chatbot ships (Phase 5, currently deferred per D-002), its API endpoint must enforce three independent limits via Upstash Redis: per-IP (10 req/60s), per-session (20 messages/conversation), per-day (1 new conversation/IP/day) — do not collapse these into a single counter (AR-17, REQ-F-066). The `booking_webhook` limiter (IP-keyed, 60 req/60s, fail-open on Redis outage) is live today on `/api/booking-webhook`, checked after HMAC verification.
...
- **Chatbot message logging**: Once the chatbot ships (Phase 5, currently deferred per D-002), never log chatbot message content — log only `{ request_id, session_id, message_count, tokens_used, cache_hit, model_id, response_time_ms }`; logging user message text would be a GDPR violation (architecture.md §3.2).
- **Rendering strategy**: All Czech + English pages are SSG (built at deploy time) — there is currently no ISR anywhere in the app; update this line if that changes. API routes (`/api/contact`, `/api/newsletter`, `/api/newsletter/confirm`, `/api/booking-webhook`, and `/api/chat` once the chatbot ships) use `Cache-Control: no-store` — never add caching to API routes without security review (architecture.md §3.1).
